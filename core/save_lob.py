...
class GroupGenerate(GoThroughBookStepper):
    ...
    def _init_curr_dataset(self):
        ...
        for indxview, count in indxview_count.items():
            self.curr_dataset[indxview] = np.full((count, 2), fill_value=np.nan, dtype=np.float32)
    
    def _init_dataset(self):
        self.dataset = {}
        self.dataset['unique_orderno'] = self.unique_orderno
        ...
        self.list_to_check_valid = ['unique_orderno', 'on_ts_org', 'on_side', 'on_px', 'on_qty_org', 'on_qty_remain']
    ...
    def run(self):
        ...
        verify_dir = Path(r'D:\CNIndexFutures\timeseries\lob_indicators\verify')
        dir_to_save = verify_dir / self.date / self.symbol
        ...
        for ts_idx, ts in self.stepper:
            ...
            df_to_save = pd.DataFrame({k: v for k, v in ts_dataset.items() if k in self.list_to_check_valid})
            best_px = ts_dataset['best_px']
            df_to_save.to_parquet(dir_to_save / f'{ts}_b_{best_px[0]}_a_{best_px[1]}.parquet')
...
# @njit(types.)
# def cut_price_range(array, best_px, price_range)
