...
def TimeRangeValueOrderAmount(best_px, on_side, on_px, on_qty_remain, on_ts_org, ts, value_thresholds, time_ranges, curr_dataset):
    """
    ...
    for T in value_thresholds:  # 遍历所有金额阈值
        for time_range in time_ranges:  # 遍历所有时间范围
            ...
            for side, col in [(0, 0), (1, 1)]:
                ...
                mask = (on_side == side) & (on_px * on_qty_remain / 10000 >= T) & (on_ts_org >= time_threshold)
