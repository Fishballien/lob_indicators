...
def ValueTimeDecayOrderAmount(best_px, on_side, on_px, on_qty_org, on_qty_remain, on_ts_org, ts, value_thresholds, decay_list, curr_dataset):
    ...
    修改：
    - ask侧使用实际挂单价格
    - bid侧使用关于中间价对称的价格(symmetric_price = 2*mid_price - bid_px)
    """
    ...
    for T in value_thresholds:
        for decay in decay_list:
            ...
            # Bid 侧处理
            # 对于bid侧，先计算对称价格，然后用对称价格判断大单
            bid_idx = on_side == 0
            if np.any(bid_idx):
                bid_px = on_px[bid_idx]
                bid_qty_org = on_qty_org[bid_idx]
                symmetric_prices = 2 * mid_price - bid_px
                large_order_mask = symmetric_prices * bid_qty_org / 10000 >= T
                mask = np.zeros_like(on_side, dtype=np.bool_)
                mask[np.where(bid_idx)[0][large_order_mask]] = True
            else:
                mask = np.zeros_like(on_side, dtype=np.bool_)
            ...
            if np.any(mask):
                ...
                curr_dataset[index, 0] = weighted_sum
            else:
                curr_dataset[index, 0] = 0  # 无有效挂单记为0
...
def LargeOrderAmountByValue(best_px, on_side, on_px, on_qty_org, on_qty_remain, value_thresholds, curr_dataset):
    ...
    for T in value_thresholds:
        # Bid侧计算
        # 对于bid侧，先计算对称价格，然后用对称价格判断大单
        temp_bid_idx = on_side == 0
        if np.any(temp_bid_idx):
            bid_px = on_px[temp_bid_idx]
            bid_qty_org = on_qty_org[temp_bid_idx]
            symmetric_prices_temp = 2 * mid_price - bid_px
            large_order_mask = symmetric_prices_temp * bid_qty_org / 10000 >= T
            bid_idx = np.zeros_like(on_side, dtype=np.bool_)
            bid_idx[np.where(temp_bid_idx)[0][large_order_mask]] = True
        else:
            bid_idx = np.zeros_like(on_side, dtype=np.bool_)
            
        if np.any(bid_idx):  # 如果有满足条件的数据
            # 计算对称价格：2*mid_price - bid_px
            symmetric_prices = 2 * mid_price - on_px[bid_idx]
            curr_dataset[index, 0] = np.sum(symmetric_prices * on_qty_remain[bid_idx] / 10000)
        else:
